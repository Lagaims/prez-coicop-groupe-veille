...
- Hierarchical: division → group → class → sub-class (~500 codes at level 5)
...
::: {.column width="55%"}
**Accuracy** *(~8,000 annotated labels, 1 annotator)*

- **Human baseline: ~90% at L5** *(with internet access)*
- **Pipeline predictions** *(no external lookup)*:
  - **L1** : consumption baskets (CPI) → LCS: 67.5% / RAG: 82.7% / TTC: 87.6% / **Pipeline: 90.0%**
  - **L4** : delivered to the HBS team → LCS: 47.1% / RAG: 65.5% / TTC: 81.1% / **Pipeline: 82.6%**
  - **L5** : Eurostat standard *(TTC & RAG stop at L4)* → **Pipeline only: 79.4%**

*Accuracy likely **underestimated**: ~10% of test labels contain annotation errors.*
...
::: {.notes}
The key finding is that the hybrid ensemble outperforms any single method. The ~10pp gap between human (90%) and pipeline (79.4%) at level 5 is partly explained by context asymmetry: human annotators can search the web, while the pipeline runs on an internal server with no external access. Accuracy figures are also conservative — roughly 10% of the test set annotations contain errors, so the pipeline's true accuracy is likely higher. The consensus shortcut reduces LLM calls significantly while preserving accuracy on agreed cases.
:::
...
- **Benchmark** all methods against manual annotations using **LLM-as-Judge**
:::
...
An ensemble of complementary methods, arbitrated by an LLM with production-grade safeguards, offers a viable path forward — not just for COICOP, but for **any survey facing the same constraint**. TTC can be retrained for any language with an annotated dataset — that portability is by design in the TTC framework.
