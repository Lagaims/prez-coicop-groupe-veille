...
::: {.column width="55%"}
**Coverage breakdown** *(pipeline run 2026-05-19, n = 8 033)*

| Method | Share |
|--------|------:|
| Regex (early exit) | 5% |
| Consensus shortcut (no LLM call) | 7% |
| LLM-as-Judge arbitration | 85% |

Of LLM-arbitrated cases, **3.5% flagged for expert review** (LLM confidence ≤ 3/5).

**Accuracy** *(LLM-as-Judge output vs manual annotation)*
...
- The **TTC classifier drives 72%** of final codes; RAG and LLM-only cover the hard cases
- **Consensus shortcut** (7% of obs) reaches 88% exact accuracy at zero LLM cost
- **RAG and LLM-only** cases score ~33% exact — these are the hardest, most ambiguous labels where all methods diverge
- Accuracy drops across levels: –13 pp from L1 → exact match
